...
9. [KPIs par Catégorie de Personne](#9-kpis-par-catégorie-de-personne)
...
## 9. KPIs par Catégorie de Personne

Ces KPIs analysent les opérations en fonction des catégories de personnes impliquées (Plaisancier, Pêcheur, Migrant, etc.).

### 9.0 Distinction Migrant / Clandestin

**IMPORTANT - Analyse des données de la base**

L'analyse de la base de données révèle que "Migrant" et "Clandestin" ne sont **PAS** deux catégories distinctes mais représentent une **transition de nomenclature** :

| Aspect | Clandestin | Migrant |
|--------|------------|---------|
| **Période** | 2009-2020 (legacy SECMAR) | 2019-2025 (SeaMIS moderne) |
| **Volume total** | 22,086 personnes (1,649 ops) | 209,864 personnes (6,945 ops) |
| **Événement principal** | EvaSan + incidents variés | Immigration clandestine (91.8%) |
| **CROSS principal** | Gris-Nez + Réunion/Mayotte | Gris-Nez (81.5%) + Sud océan Indien |

**Évolution temporelle (point pivot 2021) :**
```
CLANDESTINS: 2009-2020 → pic 2020 (538 ops) puis effondrement 2021 (20 ops)
MIGRANTS:    2019-2025 → explosion 2021 (1,705 ops), stable ~1000/an ensuite
```

**Conclusion :**
- C'est une **transition de nomenclature** (SECMAR → SeaMIS en 2021)
- **Pas une distinction de flux** (entrant/sortant)
- **Recommandation** : Regrouper les deux catégories pour les analyses historiques

**Définition officielle SECMAR :**
- "Clandestin" = personne en situation irrégulière ou ne figurant pas sur la liste des personnes embarquées

### 9.1 Comptage par Catégorie

| Attribut | Valeur |
|----------|--------|
| **Nom** | Répartition par catégorie de personne |
| **Description** | Nombre de personnes par catégorie avec % annuel |
| **Vue SQL** | `v_kpi_categorie_personne_comptage` |
| **Colonnes** | `categorie_personne`, `annee`, `total_personnes`, `pct_total_annee` |

**Catégories disponibles :**
- Plaisancier français
- Pratiquant loisirs nautiques
- Pêcheur français / Pêcheur amateur
- Commerce français / Marin étranger
- Migrant / Clandestin
- Autre / Toutes catégories

---

### 9.2 Taux de Sauvetage par Catégorie

| Attribut | Valeur |
|----------|--------|
| **Nom** | Taux de personnes saines et sauves par catégorie |
| **Description** | Application du KPI officiel SECMAR ventilé par catégorie |
| **Formule** | `saines_sauves_categorie / prises_en_compte_categorie × 100` |
| **Vue SQL** | `v_kpi_taux_sauvetage_categorie` |
| **Colonnes clés** | `taux_saines_sauves`, `taux_mortalite`, `indice_gravite` |
...
SELECT
    categorie_personne,
    annee,
    nombre_saines_sauves,
    nombre_prises_en_compte,
    ROUND(nombre_saines_sauves::NUMERIC / NULLIF(nombre_prises_en_compte, 0) * 100, 2) AS taux_saines_sauves
FROM v_kpi_taux_sauvetage_categorie;
```

---

### 9.3 Évolution Temporelle par Catégorie

| Attribut | Valeur |
|----------|--------|
| **Nom** | Tendance mensuelle par catégorie |
| **Description** | Évolution du nombre de personnes par catégorie |
| **Vue SQL** | `v_kpi_evolution_categorie_mensuel` |
| **Granularité** | Mensuelle |

---

### 9.4 Benchmark CROSS par Catégorie

| Attribut | Valeur |
|----------|--------|
| **Nom** | Comparaison CROSS sur les catégories |
| **Description** | Ranking et métriques par CROSS pour chaque catégorie |
| **Vue SQL** | `v_kpi_cross_categorie_benchmark` |
| **Colonnes** | `cross_name`, `categorie_personne`, `total_personnes`, `rank_volume_categorie` |

---

### 9.5 Métriques Sans Clandestins (Officiel Ministère)

| Attribut | Valeur |
|----------|--------|
| **Nom** | KPIs officiels ministériels |
| **Description** | Métriques calculées en excluant les clandestins et migrants |
| **Vue SQL** | `v_kpi_securite_sans_clandestins` |
| **Note** | Correspond aux chiffres publiés par le ministère |

Le ministère calcule systématiquement ses statistiques en **deux versions** :
1. **Avec clandestins** : Statistiques complètes
2. **Sans clandestins** : Statistiques officielles publiées
...
SELECT
    periode,
    cross_name,
    total_personnes_sans_clandestins,
    taux_saines_sauves_sans_clandestins,
    total_clandestins_migrants  -- Pour comparaison
FROM v_kpi_securite_sans_clandestins;
```

---

### 9.6 Carte Géographique des Catégories

| Attribut | Valeur |
|----------|--------|
| **Nom** | Données géographiques pour visualisation |
| **Description** | Coordonnées et métriques par zone pour carte interactive |
| **Vue SQL** | `v_kpi_categorie_geographique` |
| **Colonnes** | `latitude`, `longitude`, `categorie_personne`, `total_personnes`, `taux_saines_sauves` |
| **Page Streamlit** | `app/pages/12_Carte_Categories.py` |
...
| `v_kpi_categorie_personne_comptage` | Comptage par catégorie | `sql/views_kpi.sql` |
| `v_kpi_taux_sauvetage_categorie` | Taux sauvetage par catégorie | `sql/views_kpi.sql` |
| `v_kpi_evolution_categorie_mensuel` | Évolution mensuelle par catégorie | `sql/views_kpi.sql` |
| `v_kpi_cross_categorie_benchmark` | Benchmark CROSS/catégorie | `sql/views_kpi.sql` |
| `v_kpi_securite_sans_clandestins` | Métriques sans clandestins | `sql/views_kpi.sql` |
| `v_kpi_categorie_geographique` | Carte géographique catégories | `sql/views_kpi.sql` |
...
| Fichier | Contenu |
|---------|---------|
| `sql/views_kpi.sql` | Définition des 15 vues SQL |
...
| `app/pages/12_Carte_Categories.py` | Carte géographique catégories |
